    ...
    (let ((bg-back   '(:background back))
      ...
      (mapcar (lambda (face) (apply 'create-face-spec face))
              `(;; basic
                ...
                ;; helm
                (helm-source-header (,@fg-base01 ,@bg-base02 :height 1.2 ,@fmt-none))
                (helm-ff-directory (,@fg-cyan :background nil))
                (helm-candidate-number (,@fmt-undr ,@fg-blue :background nil))
                (helm-ff-file (:inherit default))
                (helm-ff-executable (,@fg-green))
                (helm-selection (,@bg-blue ,@fg-base02 ,@fmt-none))
                ;; iedit
                (iedit-occurrence (,@fg-base3 ,@bg-violet))
                ;; eldoc
                (eldoc-highlight-function-argument (,@fmt-undr ,@fg-cyan))
                ;; mmm-mode
                (mmm-code-submode-face (:background nil ,@fmt-ital))
                (mmm-output-submode-face (:background nil ,@fmt-undi))
                ...
                ;; wanderlust
                (wl-highlight-folder-few-face (,@fg-red))
                (wl-highlight-folder-many-face (,@fg-red))
                (wl-highlight-folder-path-face (,@fg-orange))
                (wl-highlight-folder-unread-face (,@fg-blue))
                (wl-highlight-folder-zero-face (,@fg-base0))
                (wl-highlight-folder-unknown-face (,@fg-blue))
                (wl-highlight-message-citation-header (,@fg-red))
                (wl-highlight-message-cited-text-1 (,@fg-red))
                (wl-highlight-message-cited-text-2 (,@fg-green))
                (wl-highlight-message-cited-text-3 (,@fg-blue))
                (wl-highlight-message-cited-text-4 (,@fg-blue))
                (wl-highlight-message-header-contents-face (,@fg-green))
                (wl-highlight-message-headers-face (,@fg-red))
                (wl-highlight-message-important-header-contents (,@fg-green))
                (wl-highlight-message-header-contents (,@fg-green))
                (wl-highlight-message-important-header-contents2 (,@fg-green))
                (wl-highlight-message-signature (,@fg-green))
                (wl-highlight-message-unimportant-header-contents (,@fg-base0))
                (wl-highlight-summary-answered-face (,@fg-blue))
                (wl-highlight-summary-disposed-face (,@fg-base0 ,@fmt-ital))
                (wl-highlight-summary-new-face (,@fg-blue))
                (wl-highlight-summary-normal-face (,@fg-base0))
                (wl-highlight-summary-thread-top-face (,@fg-yellow))
                (wl-highlight-thread-indent-face (,@fg-magenta))
                (wl-highlight-summary-refiled-face (,@fg-base0))
                (wl-highlight-summary-displaying-face (,@fmt-undb))
                ;; calfw
                (cfw:face-title (,@fg-yellow ,@fmt-bold :height 1.5 :inherit variable-pitch))
                (cfw:face-header (,@fg-base0))
                (cfw:face-default-content (,@fg-base0))
                (cfw:face-select (,@bg-base02))
                (cfw:face-day-title (,@fg-base1))
                (cfw:face-default-day (:inherit cfw:face-day-title))
                (cfw:face-today (,@fmt-bold ,@bg-base02))
                (cfw:face-today-title (,@fmt-bold ,@bg-base02 ,@fg-magenta))
                (cfw:face-saturday (,@fg-orange))
                (cfw:face-sunday (,@fg-red))
                (cfw:face-holiday (,@fg-red))
                (cfw:face-toolbar (,@fg-base02 ,@bg-base02))
                (cfw:face-toolbar-button-off (,@fg-base01 ,@bg-base02))
                (cfw:face-toolbar-button-on (,@fg-base1 ,@bg-base02))
                (cfw:face-grid (,@fg-base01))
                (cfw:face-periods (,@fg-cyan))
                (cfw:face-annotation (,@fg-yellow :inherit cfw:face-day-title))
                (cfw:face-disable (,@fg-base01 :inherit cfw:face-day-title))
                ;; minimap
                ...
                ;; tabbar
                (tabbar-selected (,@bg-blue ,@fg-base02))
                (tabbar-unselected (,@bg-base0 ,@fg-base02))
                (tabbar-modified (,@bg-green ,@fg-base02))
                ;; smartparens
                ...
                (erc-bold-face (,@fmt-bold))
                (erc-current-nick-face (,@fg-blue ,@fmt-bold))
                (erc-dangerous-host-face (:inherit font-lock-warning))
                (erc-default-face (,@fg-base0 ,@bg-base03 ,@fmt-none))
                ...
                (erc-input-face (:foreground ,@fg-base01))
                (erc-keyword-face (,@fmt-bldi ,@fg-yellow))
                (erc-my-nick-face (:foreground ,@fg-blue))
                (erc-nick-default-face (,@fmt-none ,@fg-cyan))
                (erc-notice-face (,@fmt-none ,@fg-blue))
                (erc-pal-face (,@fg-orange ,@fmt-bold))
                (erc-prompt-face (,@fg-orange ,@bg-base03 ,@fmt-bold))
                (erc-timestamp-face (:foreground ,@fg-base01))
                ...
                (e2wm:face-history-list-select2 (,@fg-cyan ,bg-base03))
                ;;flyspell
